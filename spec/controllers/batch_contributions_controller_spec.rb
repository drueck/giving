...
describe BatchContributionsController do
  ...
  describe "#index" do
    before(:each) do
      @contribution = FactoryGirl.create(:contribution, batch_id: @batch.id)
  ...
  describe "#create" do
    context "when the required attributes are supplied" do
      before(:each) do
        ...
        @attrs = { contributor_id: @contributor.id, batch_id: @batch.id,
          amount: 9.99, date: "12/1/2013" }
      end
      ...
      context "when requesting js format" do
        before(:each) do
          post :create, batch_id: @batch.id, contribution: @attrs, format: :js
        end
